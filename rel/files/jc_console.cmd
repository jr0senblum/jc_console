...
@set node_name=jc
@set long_name=jcache@127.0.0.1
@rem remove -name nodename from vm.args
...
:install
@%erts_bin%\erlsrv.exe add %service_name% -onfail restart_always -c "Erlang node %node_name% in %node_root%" -name %long_name% -w %node_root% -m %node_root%\bin\start_erl.cmd -args " ++ %node_name% ++ %node_root%" -stopaction "init:stop()."
@goto :EOF
...
:console
@start %erts_bin%\werl.exe -name %long_name% -boot %releases_dir%\%release_version%\%node_name% -config %releases_dir%\%release_version%\sys.config -args_file %vm_args%
@goto :EOF
...
start %erts_bin%\werl.exe -boot %releases_dir%\%release_version%\start_clean -remsh %long_name% -name test -setcookie %erlang_cookie%
@goto :EOF
